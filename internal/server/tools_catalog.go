...
func (r *Runtime) registerConsolidatedToolsCatalog(s *mcp.Server) {
	...
	executeBranches := map[string]actionSchemaBranch{
		"run": {Description: "执行 argv+shell=false、command、项目 task，或一次性 runtime+script，四种模式互斥。argv 按元素直接传递；python/node 源码经 stdin+EOF 直接执行且不经过 shell，sqlite 对 Workspace 内现有数据库执行只读单查询并返回结构化行。authorization_request 只在首次建立同一对话工作包 grant 时提供；后续使用 authorization_grant_id。", Properties: map[string]any{
			"command": stringSchema("Workspace 内待执行的简单命令"), "task": stringSchema("项目任务名称，与 argv/command/runtime+script 互斥"),
